...
logger = logging.getLogger('quick_tomopy')
logger.setLevel(logging.INFO)
formatter = logging.Formatter('%(asctime)s %(name)-12s %(levelname)-8s %(message)s')
fh = handlers.RotatingFileHandler('quick_tomopy.log')
...
def write_als_832h5(rec, file_name, file_data, group_data, out_path, step=1):
    ...
    logger = logging.getLogger('quick_tomopy.write_als_832h5')
...
def quick_tomo_recon(argv):
    """
    Reconstruct subset slices (sinograms) equally spaced within tomographic
    dataset
    """

    logger = logging.getLogger('quick_tomopy.quick_tomo_recon')
